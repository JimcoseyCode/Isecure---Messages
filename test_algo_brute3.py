...
def test_all():
    ...
    # Let's see if varying the shift format works
    for L in [4096]:
        for shift_fmt in ["{ctxA}:{idx}:{sk}", "{ctxA}:{idx}:{sk_upper}", "{sk}:{idx}:{ctxA}", "{ctxA}|{idx}|{sk}"]:
            ...
            for k in range(0, len(payEnc), 3):
                ...
                if shift_fmt == "{ctxA}:{idx}:{sk}":
                    s_str = f"{ctxA}:{idx}:{sk}"
                elif shift_fmt == "{ctxA}:{idx}:{sk_upper}":
                    s_str = f"{ctxA}:{idx}:{sk.upper()}"
                elif shift_fmt == "{sk}:{idx}:{ctxA}":
                    s_str = f"{sk}:{idx}:{ctxA}"
                elif shift_fmt == "{ctxA}|{idx}|{sk}":
                    s_str = f"{ctxA}|{idx}|{sk}"
                    
                shift = int(sha3h(s_str.encode()), 16)
                ci = (v - shift) % L
                out_indices.append(ci)
            
            for i in range(len(out_indices) - 5):
                if len(set(out_indices[i:i+6])) == 1:
                    print(f"FOUND MATCH with fmt {shift_fmt}!")
