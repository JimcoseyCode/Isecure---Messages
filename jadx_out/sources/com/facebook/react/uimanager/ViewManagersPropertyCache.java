    ...
    private static void createPropSetters(ReactPropGroup reactPropGroup, Method method, Class<?> cls, Map<String, PropSetter> map) {
        ...
        if (cls == Dynamic.class) {
            while (i10 < strArrNames.length) {
                map.put(strArrNames[i10], new DynamicPropSetter(reactPropGroup, method, i10));
                i10++;
            }
            return;
        }
        if (cls == Integer.TYPE) {
            while (i10 < strArrNames.length) {
                if ("Color".equals(reactPropGroup.customType())) {
                    map.put(strArrNames[i10], new ColorPropSetter(reactPropGroup, method, i10, reactPropGroup.defaultInt()));
                } else {
                    map.put(strArrNames[i10], new IntPropSetter(reactPropGroup, method, i10, reactPropGroup.defaultInt()));
                }
                i10++;
            }
            return;
        ...
        if (cls == Float.TYPE) {
            while (i10 < strArrNames.length) {
                map.put(strArrNames[i10], new FloatPropSetter(reactPropGroup, method, i10, reactPropGroup.defaultFloat()));
                i10++;
            }
            return;
        }
        if (cls == Double.TYPE) {
            for (int i11 = 0; i11 < strArrNames.length; i11++) {
                map.put(strArrNames[i11], new DoublePropSetter(reactPropGroup, method, i11, reactPropGroup.defaultDouble()));
            }
            return;
        }
        if (cls == Integer.class) {
            while (i10 < strArrNames.length) {
                if ("Color".equals(reactPropGroup.customType())) {
                    map.put(strArrNames[i10], new BoxedColorPropSetter(reactPropGroup, method, i10));
                } else {
                    map.put(strArrNames[i10], new BoxedIntPropSetter(reactPropGroup, method, i10));
                }
                i10++;
            }
            return;
    ...
    static abstract class PropSetter {
        ...
        public void updateShadowNodeProp(ReactShadowNode reactShadowNode, Object obj) {
            ...
            try {
                ...
                AbstractC2325a.j(ViewManager.class, "Error while updating prop " + this.mPropName, th);
