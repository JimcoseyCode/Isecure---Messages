use sha3::{Sha3_256, Digest};
use sha2::Sha256;
use hmac::{Hmac, Mac};
use rayon::prelude::*;
use std::sync::atomic::{AtomicBool, Ordering};

fn hx(s: &str) -> Vec<u8> {
    (0..s.len()).step_by(2).map(|i| u8::from_str_radix(&s[i..i + 2], 16).unwrap()).collect()
}

fn hmac_sha256(key: &[u8], data: &[u8]) -> [u8; 32] {
    let mut m = <Hmac<Sha256>>::new_from_slice(key).unwrap();
    Mac::update(&mut m, data);
    m.finalize().into_bytes().into()
}

fn sas(shared: &[u8]) -> String {
    let salt = Sha256::digest(b"OV_PQHybrid_Salt_v1");
    let prk = hmac_sha256(&salt, shared);
    let mut info = b"OV_PQHybrid_SAS_v1".to_vec();
    info.push(1);
    let t = hmac_sha256(&prk, &info);
    t[..6].iter().map(|b| format!("{b:02X}")).collect()
}

fn get_permutations<T: Clone>(arr: &[T]) -> Vec<Vec<T>> {
    if arr.is_empty() {
        return vec![vec![]];
    }
    let mut res = Vec::new();
    for i in 0..arr.len() {
        let mut rest = arr.to_vec();
        let el = rest.remove(i);
        for p in get_permutations(&rest) {
            let mut p2 = vec![el.clone()];
            p2.extend(p);
            res.push(p2);
        }
    }
    res
}

#[derive(Clone)]
struct Element {
    name: &'static str,
    raw: Vec<u8>,
}

fn main() {
    let target = "350E6E63A652";
    
    let ss_pq = hx("fd7dfab2f10a2e16e0c20135c187ad0b18c4bc54a6b71197cd19eeb1ea01bc11");
    let ss_ec = hx("96beb64dcdd4ebd732edc97dab6f87e80ce2277540b85dcc86a16480c4be4837");
    let ct = hx("");
    let pk = hx("");
    
    let ct_pq = ct[..1088].to_vec();
    let ct_ec = ct[1088..].to_vec();
    let pk_pq = pk[..1184].to_vec();
    let pk_ec = pk[1184..].to_vec();

    let make_el = |name: &'static str, val: &[u8]| -> Element {
        Element { name, raw: val.to_vec() }
    };

    let all_perms = get_permutations(&vec![
        make_el("ss_pq", &ss_pq),
        make_el("ss_ec", &ss_ec),
        make_el("ct_ec", &ct_ec),
        make_el("pk_ec", &pk_ec),
    ]);

    let found = AtomicBool::new(false);

    all_perms.par_iter().for_each(|p| {
        if found.load(Ordering::Relaxed) { return; }
        
        let mut preimage = Vec::new();
        for el in p {
            preimage.extend_from_slice(&el.raw);
        }
        preimage.extend_from_slice(b"\\.//^\\");
        
        let mut shared = [0u8; 32];
        shared.copy_from_slice(&Sha3_256::digest(&preimage));
        
        if sas(&shared) == target {
            found.store(true, Ordering::Relaxed);
            let names: Vec<_> = p.iter().map(|e| e.name).collect();
            println!("MATCH FOUND for SHA3-256 direct (X-Wing)!");
            println!("Order: {:?}", names);
        }
    });

    if !found.load(Ordering::Relaxed) {
        println!("No match found in X-Wing brute force.");
    }
}
